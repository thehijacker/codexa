...
let lastContents = null; // most recently rendered Contents object (for highlight fallback)
...
// Keydown forwarding is handled automatically by @intity/epub-js via rendition events.
function attachIframeWheel(contents) {
...
function estimateBookTotal() {
  const len = book?.sections?.size || 1;
  let total = 0;
...
// Safely extract a page number from the fork's displayed.page value,
// which may be a plain number, an object, or an array depending on version.
function extractPage(val) {
  if (typeof val === 'number') return val;
  if (Array.isArray(val))     return val[0] ?? 0;
  if (val && typeof val === 'object') return val.page ?? val.index ?? val.num ?? 0;
  return parseInt(val) || 0;
}

// Update all overlay slots from current state
function updateStatusBar(location) {
  if (!window._v2LocLogged) {
    window._v2LocLogged = true;
    console.log('[v2 location.start.displayed]', JSON.stringify(location?.start?.displayed));
    console.log('[v2 location.end.displayed]',   JSON.stringify(location?.end?.displayed));
  }
  // @intity/epub-js reports displayed.page as 0-indexed; add 1 to make it 1-indexed.
  const startPage  = extractPage(location?.start?.displayed?.page) + 1;
  const totalPages = location?.start?.displayed?.total ?? 0;
  const endPage    = extractPage(location?.end?.displayed?.page) + 1;
  const isTwoPage  = totalPages > 0 && endPage !== startPage;
  ...
  toc.forEach(item => {
    ...
    btn.addEventListener('click', () => {
      ...
      setTimeout(async () => {
        ...
        const spineItem = book.sections?.get(hrefBase);
        let displayHref;
        if (spineItem) {
...
function getActiveContents() {
  let list = [];
  try {
    list = rendition?.getContents() || [];
    if (!list.length) throw new Error('empty');
  } catch {
    list = [];
    rendition?.manager?.views?.forEach?.(v => { if (v?.contents) list.push(v.contents); });
    if (!list.length && lastContents?.document) list = [lastContents];
  }
  return list;
}
...
function clearSearchHighlights() {
  ...
  if (!rendition) return;
  try {
    getActiveContents().forEach(content => {
...
function applyDomSearchHighlight(query) {
  ...
  try {
    ...
    if (!contents.length) { console.warn('[search] highlight: no active contents found'); return; }
...
async function jumpToSearchResult(cfi, href, query) {
  ...
  const targetItem  = book.sections?.get(href);
  ...
  if (sameChapter) {
    ...
    try { await rendition.display(cfi); } catch { try { await rendition.display(navCfi); } catch {} }
  } else {
...
async function runSearch(query) {
  ...
  // Collect all sections (fork uses book.sections Map instead of book.spine)
  const sections = [];
  book.sections.forEach(item => sections.push(item));
...
function updateProgress(location) {
  ...
  const inSearch = preSearchCfi !== null; // freeze position while browsing search results
  let pct = 0;
  if (cfi && book.locations?.size > 0) {
    pct = book.locations.percentageFromCfi(cfi) || 0;
  } else {
    const idx   = parseInt(location?.start?.index) || 0;
    const total = book?.sections?.size || 1;
    pct = idx / total;
  }
  // Always track spine index (needed for status bar accuracy regardless of ready/search state)
  currentSpineIndex = parseInt(location?.start?.index) ?? currentSpineIndex;

  // Only update save-state after init is done, and never during search navigation.
  // During startup, relocated fires with the page-start CFI which is earlier than
  // the exact saved character offset we want to preserve.
  if (isReady && !inSearch) {
    currentCfi = cfi || '';
    currentPct = pct;
    console.log('[pos] relocated (ready) cfi:', (cfi||'').slice(0,60));
  } else if (!isReady && !inSearch) {
    // Pre-init: update display pct so status bar stats show non-zero values immediately
    currentPct = pct;
    console.log('[pos] relocated (pre-ready) cfi:', (cfi||'').slice(0,60));
  } else {
    console.log('[pos] relocated (search-frozen) cfi:', (cfi||'').slice(0,60));
  }
  ...
  // Save on chapter boundary — only outside search mode and after init
  if (isReady && !inSearch && lastChapterHref !== null && href && href !== lastChapterHref) {
    saveProgress();
  }
...
function initLocations() {
  ...
  if (cached) {
    try {
      ...
      if (currentCfi && book.locations.size > 0) {
...
async function startRendition(displayCfi = null) {
  ...
  rendition.hooks.content.register((contents) => {
    lastContents = contents; // cache for highlight fallback
    // The manager constructor ignores gap — set it here before every render so
    // layout.calculate() uses our margin instead of the auto-calculated szw/12.
    if (rendition.manager?.settings) rendition.manager.settings.gap = prefs.margin * 2;
    attachIframeWheel(contents);
  ...
  rendition.on('rendered', (view) => {
    ...
    // Prevent horizontal elastic overscroll inside the iframe during swipes.
    const iframe = view.element?.querySelector('iframe') || view.element;
    if (iframe) iframe.style.touchAction = 'pan-y';
  });

  // @intity/epub-js forwards iframe DOM events via the rendition.
  // Relay keydown so our host-page keyboard handler fires when the iframe has focus.
  ...
  // Relay touch events from inside the iframe, offset-adjusted to host-page coordinates.
  ...
  rendition.on('touchend', (e) => {
    handleTouchEnd({
      changedTouches: [{
        clientX: e.changedTouches[0].clientX + iframeOffX,
        clientY: e.changedTouches[0].clientY + iframeOffY,
      }],
    });
  });
  ...
  rendition.on('relocated', () => {
    ...
    if (searchNav.phase === 'first') {
      ...
      // Wait 500ms for font/layout reflow, then navigate to the exact CFI position.
      const { cfi, navCfi } = searchNav;
      searchNav.phase = 'second';
      setTimeout(() => { rendition.display(cfi).catch(() => rendition.display(navCfi).catch(() => {})); }, 500);
      ...
      setTimeout(() => applyDomSearchHighlight(query), 500);
    }
  });
...
window.addEventListener('resize', debounce(async () => {
  ...
  rendition.resize(epubViewer.clientWidth, epubViewer.clientHeight);
  if (isReady && pctBeforeResize > 0 && book.locations?.size > 0) {
...
async function init() {
  ...
  try {
    ...
    book.opened.then(() => initLocations()).catch(() => {});
    loadingOverlay.classList.add('hidden');

    // epub.js display(cfi) with char-offset CFIs snaps to wrong page — seek forward by pct.
    if (localProgress?.percentage != null && book.locations.size > 0) {
    ...
    if (syncTarget?.percentage != null) {
      try {
        ...
        if (dfMatch) {
          ...
          const spineItem = [...(book.sections?.values() ?? [])].find(s => s.index === spineIdx);
          if (spineItem?.href) {
            ...
            if (!book.locations.size) {
              await book.locations.generate(1000);
              try { localStorage.setItem(key, book.locations.save()); } catch { /* quota */ }
            }
            if (book.locations.size > 0) await seekToPercentage(syncTarget.percentage);
          }
        } else {
    ...
    // Refresh status bar now that currentPct/currentCfi are fully settled
    renderStatusSlots();
  } catch (err) {
