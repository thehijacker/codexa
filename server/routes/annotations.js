...
router.get('/:bookId', (req, res) => {
  ...
  const rows = db.prepare(
    'SELECT id, cfi, pct, text, note, color, created_at FROM annotations WHERE user_id = ? AND book_id = ? AND deleted = 0 ORDER BY pct ASC'
...
router.post('/:bookId', (req, res) => {
  ...
  const result = db.prepare(
    "INSERT INTO annotations (user_id, book_id, cfi, pct, text, note, color, origin, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'web', strftime('%s','now'))"
...
router.put('/:bookId/:id', (req, res) => {
  ...
  db.prepare("UPDATE annotations SET note = ?, color = ?, updated_at = strftime('%s','now') WHERE id = ?")
...
router.delete('/:bookId/:id', (req, res) => {
  ...
  if (bookorbit.isEnabled(req.user.id)) {
    db.prepare("UPDATE annotations SET deleted = 1, updated_at = strftime('%s','now') WHERE id = ?").run(an.id);
    bookorbit.triggerSync(req.user.id, Number(req.params.bookId));
  } else {
